...
def get_intervention_from_path(path):
    # `path` is like
    # /mnt/database/nimodipine-rct-data/interventions/fax/P84027/fax.pdf
    method, practice_id = re.search(r"(fax|email)/(.*)/.*", path).groups()
    try:
        contact = InterventionContact.objects.get(practice_id=practice_id)
        return contact.intervention_set.get(method=method[0])
    except InterventionContact.DoesNotExist:
        logging.error("Could not find contact for practice %s", practice_id)
    except Intervention.DoesNotExist:
        logging.error(
            "Could not find intervention %s for practice %s", method, practice_id
        )
...
def send_fax_message(msg_path, recipient=None, dry_run=False):
    ...
    subject = "about your nimodipine prescribing"
    kwargs = {
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options["method"]:
            ...
            methods = ["email", "fax"]
        prompt = "Sending messages using method(s) {}".format(methods)
        ...
        if really.strip().lower() == "y":
            for method in methods:
                base_path = os.path.join(settings.DATA_DIR, method)
                if options["practice"]:
                ...
                for practice_id in practices:
                    ...
                    if method == "email":
                        send_email_message(
                            msg_path, options["test_recipient"], options["dry_run"]
                        )
                    elif method == "fax":
                        send_fax_message(
                            msg_path, options["test_recipient"], options["dry_run"]
