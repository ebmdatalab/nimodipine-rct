...
def get_intervention_from_path(path):
    ...
    method, practice_id = re.search(
        r"(fax|email)/(.*)/.*", path).groups()
    try:
        ...
        logging.error(
            "Could not find intervention %s for practice %s",
            method, practice_id)
...
def send_email_message(msg_path, recipient=None, dry_run=False):
    ...
    with open(email_path, 'r') as body_f:
        ...
        if recipient:
            ...
            to = recipient
        subject = 'Information about your nimodipine prescribing from OpenPrescribing.net'
